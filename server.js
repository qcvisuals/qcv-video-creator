...
app.post('/generate', upload.array('photos', 25), async (req, res) => {
  ...
  try {
    ...
    for (let f = 0; f < actualFrames; f++) {
      ...
      // Background
      filterParts.push('color=' + bgColor + ':size=' + W + 'x' + H + ':rate=30,format=yuv420p [vbg]');
      ...
      const ffArgs = ['-y'];
      ffArgs.push('-f', 'lavfi', '-i', 'color=' + bgColor + ':size=' + W + 'x' + H + ':rate=30,format=yuv420p');
