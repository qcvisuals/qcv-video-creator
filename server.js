...
app.post('/api/generate-video', (req, res, next) => { req.jobId = uuidv4(); next(); },
  ...
  async (req, res) => {
    try {
      ...
      const { address, price, beds, baths, sqft, tagline, agentName, agentPhone, musicMood, videoFormat } = req.body;
      ...
      jobs.set(jobId, { status: 'processing', progress: 0, message: 'Starting...' });
      res.json({ jobId });
      processVideo({ jobId, photos, logoFile, address, price, beds, baths, sqft, tagline, agentName, agentPhone, musicMood, videoFormat });
...
async function processVideo({ jobId, photos, logoFile, address, price, beds, baths, sqft, tagline, agentName, agentPhone, musicMood, videoFormat }) {
  ...
  try {
    ...
    update('processing', 10, 'Preparing photos...');
    const segments = [];
    for (let i = 0; i < photos.length; i++) {
      const segOut = path.join(workDir, 'seg_' + i + '.mp4');
      const fps = 30, dur = 3, frames = dur * fps;
      const zoompan = 'zoompan=z=\'min(zoom+0.0008,1.08)\':x=\'iw/2-(iw/zoom/2)\':y=\'ih/2-(ih/zoom/2)\':d=' + frames + ':s=' + w + 'x' + h + ':fps=' + fps;
      let filter = '[0:v]scale=' + (w*2) + ':' + (h*2) + ',' + zoompan;
      const txt = getTextOverlay(i, address, price, beds, baths, sqft, tagline, w, h);
      if (txt) filter += ',' + txt;
      filter += '[v]';
      await runFFmpeg(['-loop','1','-t',String(dur),'-i',photos[i],'-filter_complex',filter,'-map','[v]','-c:v','libx264','-preset','fast','-pix_fmt','yuv420p','-r',String(fps),'-t',String(dur),'-y',segOut]);
      segments.push(segOut);
      update('processing', 10 + Math.floor((i/photos.length)*50), 'Processing photo ' + (i+1) + ' of ' + photos.length + '...');
    }
    update('processing', 65, 'Creating end card...');
    const endCard = path.join(workDir, 'endcard.mp4');
    const ec = buildEndCard(agentName, agentPhone, w, h);
    await runFFmpeg(['-f','lavfi','-i','color=c=black:s='+w+'x'+h+':d=3:r=30','-filter_complex',ec,'-map','[v]','-c:v','libx264','-preset','fast','-pix_fmt','yuv420p','-t','3','-y',endCard]);
    segments.push(endCard);
    update('processing', 70, 'Joining clips...');
    const concatList = path.join(workDir, 'concat.txt');
    fs.writeFileSync(concatList, segments.map(s => "file '" + s + "'").join('\n'));
    const concatOut = path.join(workDir, 'concat.mp4');
    await runFFmpeg(['-f','concat','-safe','0','-i',concatList,'-c:v','libx264','-preset','fast','-pix_fmt','yuv420p','-y',concatOut]);
    update('processing', 80, 'Adding logo...');
    ...
    if (logoFile && fs.existsSync(logoFile)) {
      ...
      await runFFmpeg(['-i',concatOut,'-i',logoFile,'-filter_complex','[1:v]scale='+ls+':-1[logo];[0:v][logo]overlay=W-w-'+pad+':H-h-'+pad+'[v]','-map','[v]','-c:v','libx264','-preset','fast','-pix_fmt','yuv420p','-y',logoOut]);
      videoWithLogo = logoOut;
    }
    update('processing', 90, 'Adding music...');
    ...
    jobs.set(jobId, { ...jobs.get(jobId), status: 'complete', progress: 100, message: 'Video ready!', outputPath });
...
function runFFmpeg(args) {
  return new Promise((resolve, reject) => {
    ...
    ff.on('close', code => { if (code===0) resolve(); else reject(new Error('FFmpeg failed: ' + stderr.slice(-300))); });
  });
}
